...
function ReportsContent() {
  ...
  // Timeslots come enriched with professionalName, clientName, serviceName from the API
  const { data: allTimeslots = [], isLoading } = useListTimeslots();
  ...
  // By professional — use enriched professionalName directly from timeslot
  const byProfessional = useMemo(() => {
    const map: Record<string, { name: string; scheduled: number; done: number; cancelled: number }> = {};
    for (const s of timeslots) {
      const name = (s as any).professionalName || `ID ${s.professionalId}`;
      if (!map[name]) map[name] = { name: name.split(" ")[0], scheduled: 0, done: 0, cancelled: 0 };
      map[name][s.status as "scheduled" | "done" | "cancelled"]++;
    }
  ...
  // By service — use enriched serviceName directly from timeslot
  const byService = useMemo(() => {
    const map: Record<string, number> = {};
    for (const s of timeslots) {
      const name = (s as any).serviceName || `ID ${s.serviceId}`;
  ...
  // Daily trend — last N days bucketed
  ...
  // Top clients — use enriched clientName directly from timeslot
  const byClient = useMemo(() => {
    const map: Record<string, number> = {};
    for (const s of timeslots) {
      const name = (s as any).clientName || `ID ${s.clientId}`;
      map[name] = (map[name] || 0) + 1;
    }
    return Object.entries(map)
      .map(([name, count]) => ({ name, count }))
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <Card key={i}><CardContent className="p-5 h-20 animate-pulse bg-muted/40" /></Card>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Live insights across appointments, professionals, and services.
          </p>
        </div>
      ...
      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Appointments" value={timeslots.length}
          icon={CalendarCheck} color="bg-blue-100 text-blue-600" />
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={dailyTrend} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
                ...
                <Tooltip />
                <Legend iconSize={8} />
                <Line type="monotone" dataKey="done" name="Completed"
                  stroke={STATUS_COLORS.done} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="scheduled" name="Scheduled"
                  stroke={STATUS_COLORS.scheduled} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="cancelled" name="Cancelled"
                  stroke={STATUS_COLORS.cancelled} strokeWidth={2} dot={false} strokeDasharray="4 2" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Stacked by status per professional</CardDescription>
          </CardHeader>
          <CardContent>
            {byProfessional.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={Math.max(200, byProfessional.length * 48)}>
                <BarChart data={byProfessional} layout="vertical"
                  ...
                  <Tooltip />
                  <Legend iconSize={8} />
                  <Bar dataKey="done" name="Completed" stackId="a" fill={STATUS_COLORS.done} />
                  <Bar dataKey="scheduled" name="Scheduled" stackId="a" fill={STATUS_COLORS.scheduled} />
                  <Bar dataKey="cancelled" name="Cancelled" stackId="a"
                    fill={STATUS_COLORS.cancelled} radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {byService.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={Math.max(200, byService.length * 48)}>
        ...
        <CardContent>
          {byClient.length === 0 ? (
            ...
            <div className="space-y-4">
              {byClient.map((c, i) => {
                const pct = timeslots.length > 0
                  ? Math.round((c.count / timeslots.length) * 100) : 0;
                return (
